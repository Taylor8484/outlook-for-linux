 ...
 * from io.github.taylor8484.outlook_for_linux.appdata.xml file, ensuring version consistency across package files.
 *
...
async function generateReleaseInfo(projectRoot = null) {
  ...
  if (!component?.releases?.[0]?.release) {
    throw new Error(
      "No releases found in io.github.taylor8484.outlook_for_linux.appdata.xml."
    );
  }
  ...
  if (!matchingRelease) {
    throw new Error(
      `No release entry found for version ${baseVersion} in io.github.taylor8484.outlook_for_linux.appdata.xml. Please add a release entry for this version.`
    );
  }
  ...
  if (!releaseNotes.trim()) {
    throw new Error(
      `Release ${baseVersion} has no description/notes in io.github.taylor8484.outlook_for_linux.appdata.xml. Please add release notes to the <description> section for this version.`
    );
  }
...
if (require.main === module) {
  (async () => {
    try {
      ...
      console.log(
        `   io.github.taylor8484.outlook_for_linux.appdata.xml: ${versionInfo.appdata} (with release notes)`
      );
      console.log("");
      console.log(
